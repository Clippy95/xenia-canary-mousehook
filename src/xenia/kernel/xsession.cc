...
X_RESULT XSession::JoinSession(XSessionJoin* data) {
  ...
  for (uint32_t i = 0; i < data->array_count; i++) {
    ...
    XELOGI("XUID: {:016X} - Occupying {} slot", member->OnlineXUID,
...
X_RESULT XSession::LeaveSession(XSessionLeave* data) {
  ...
  for (uint32_t i = 0; i < data->array_count; i++) {
    ...
    if (!member->IsZombie()) {
      ...
      XELOGI("XUID: {:016X} - Leaving {} slot", member->OnlineXUID,
             member->IsPrivate() ? "private" : "public");
...
void XSession::PrintSessionDetails() {
  XELOGI(
      ...
      local_details_.UserIndexHost,
      local_details_.GameType ? "Standard" : "Ranked", local_details_.GameMode,
      local_details_.eState, local_details_.Nonce, local_details_.Flags,
  ...
  for (const auto& [xuid, mamber] : local_members_) {
    XELOGI(
        ...
        "Flags: {:08X}\n"
        "IsPrivate: {}\n",
        index++, mamber.OnlineXUID, mamber.UserIndex, mamber.Flags,
        mamber.IsPrivate() ? "True" : "False");
  }

...
bool PdzIsUniversalCam(uint8_t cam_type) {
  ...
  uint8_t* current_cam = kernel_memory()->TranslateVirtual<uint8_t*>(*ptr1 +
                                                                     0xB);
...
void ApplyPdzGunSway(uint32_t player, float mouse_x, float mouse_y,
  ...
  auto* gun_x = kernel_memory()->TranslateVirtual<xe::be<float>*>(
      player + kGunXOffset);
  auto* gun_y = kernel_memory()->TranslateVirtual<xe::be<float>*>(
      player + kGunYOffset);
  if (!gun_x || !gun_y) {
    return;
  }
  ...
  if (raw_mouse_x != 0.0f || raw_mouse_y != 0.0f) {
    ...
    const float center_dt = std::clamp(std::isfinite(dt) ? dt : 0.0f,
                                       1.0f / 120.0f, 1.0f / 20.0f);
    ...
    xe::GameLaunchHooks::OnPreLaunch().AddListener(
        [](xe::kernel::UserModule* module) {
          ...
          if (pattern2.empty())
              return;
          pdz_hook_valid = true;
          xe::cpu::ppc::RegisterFunctionHook<void, uint32_t, uint32_t,
                                             uint32_t, double>(
              pattern.get_first(),
              [](auto& ctx, uint32_t ptr1, uint32_t ptr2, uint32_t ptr3,
                 double a4) {
                if (!ptr3) {
                ...
                if (ptr1 == ptr2 && ptr3 == ptr1 + 0xF50) {
                  look_impulse = LimitPdzPitchImpulse(
                      ptr1, look_impulse, static_cast<float>(a4));
                }

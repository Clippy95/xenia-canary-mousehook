...
bool BattlefieldGame::DoHooks(uint32_t user_index, RawInputState& input_state,
  ...
  auto* camera_base = multi_pointer(supported_builds[game_build_].base_address,
                                    {0xB8, 0x11C, 0x20C, 0x3C, 0x8});
  if (camera_base == NULL) return false;
...
void BattlefieldGame::WeaponSwitchHandler(uint32_t user_index,
                                          RawInputState& input_state,

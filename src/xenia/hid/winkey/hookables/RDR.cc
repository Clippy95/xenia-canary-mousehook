...
bool RedDeadRedemptionGame::DoHooks(uint32_t user_index,
  ...
  if (!IsGameSupported() || supported_builds.count(game_build_) == 0) {
    return false;
  }

  auto now = std::chrono::steady_clock::now();
  long long elapsed_x = std::chrono::duration_cast<std::chrono::milliseconds>(
                            now - last_movement_time_x_)
                            .count();
  long long elapsed_y = std::chrono::duration_cast<std::chrono::milliseconds>(
  ...
  float divisor = CalculateFOVDivisor();
  HandleCoverOrMountedCamera(input_state, divisor);
  UpdateCameraPosition(input_state, divisor, base_address);

  ApplyAutoCentering(input_state);
  ApplyMountingCenter(input_state);

  return true;
}

bool RedDeadRedemptionGame::CheckPauseFlag() {
...
float RedDeadRedemptionGame::CalculateFOVDivisor() {
  if (supported_builds[game_build_].fovscale_base_address != NULL) {
    ...
    float fov = (*fovscale <= 0.5f || *fovscale > 35.f) ? 1.f : *fovscale;
    return 850.5f * fov;
  }
  return 850.5f;
}

void RedDeadRedemptionGame::HandleCoverOrMountedCamera(
    RawInputState& input_state, float divisor) {
  if (supported_builds[game_build_].cover_base_address != NULL) {
    ...
    if (cam_type && *cam_type == 9) {
      AdjustCameraCover(input_state, divisor);
    } else if (cam_type && (*cam_type == 7 || *cam_type == 6)) {
      AdjustMountedCamera(input_state, divisor);
    }
  }
}

void RedDeadRedemptionGame::AdjustCameraCover(RawInputState& input_state,
                                              float divisor) {
  ...
  camX -=
      ((input_state.mouse.x_delta * static_cast<float>(cvars::sensitivity)) /
       divisor);
  camY -=
      ((input_state.mouse.y_delta * static_cast<float>(cvars::sensitivity)) /
       divisor);

  *radian_x_cover = camX;
  *radian_y_cover = camY;

  UpdateCoverCenter(input_state);
}

void RedDeadRedemptionGame::AdjustMountedCamera(RawInputState& input_state,
                                                float divisor) {
  xe::be<uint32_t>* cover_base =
      kernel_memory()->TranslateVirtual<xe::be<uint32_t>*>(
          supported_builds[game_build_].cover_base_address);
...
void RedDeadRedemptionGame::UpdateCoverCenter(RawInputState& input_state) {
...
void RedDeadRedemptionGame::UpdateCameraPosition(
    RawInputState& input_state, float divisor, xe::be<uint32_t>* base_address) {
  ...
  degree_y = std::clamp(degree_y, -0.861205390f, 0.7153550260f);
...
void RedDeadRedemptionGame::ApplyAutoCentering(RawInputState& input_state) {
  if (supported_builds[game_build_].auto_center_strength_offset != NULL) {
    ...
    if (auto_center_strength <= 1.f &&
      ...
      auto_center_strength = std::min(auto_center_strength + 0.15f, 1.f);
      *auto_center_strength_act = auto_center_strength;
    }
  }
}

void RedDeadRedemptionGame::ApplyMountingCenter(RawInputState& input_state) {
